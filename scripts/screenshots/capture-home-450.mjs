...
// 360x640 逻辑分辨率，1.25 倍缩放输出 450x800
await page.setViewport({ width: 360, height: 640, deviceScaleFactor: 1.25 });

await page.goto(`http://127.0.0.1:${PORT}/index.html`, { waitUntil: 'networkidle0' });
await page.evaluate(() => localStorage.setItem('mp4gif_privacy_consent', '1'));
await page.reload({ waitUntil: 'networkidle0' });
await new Promise((r) => setTimeout(r, 400));
await page.screenshot({ path: path.join(outDir, '主界面-450x800.png') });

await browser.close();
server.close();
console.log('saved 主界面-450x800.png');
